...
<head>
    ...
    <style>
        ...
        :root {
            ...
            --agent-secondary: #14b8a6;
            --agent-accent: #5eead4;
            --agent-dark: #134e4a;
            --agent-light: #ccfbf1;
            --agent-gradient: linear-gradient(135deg, #0f766e 0%, #14b8a6 50%, #0d9488 100%);
            --agent-glass: rgba(15, 118, 110, 0.1);
        }
...
<body class="font-sans antialiased bg-gray-50">
    <div class="min-h-screen" x-data="{ sidebarOpen: window.innerWidth > 768, mobileSidebarOpen: false }">
        ...
        <aside class="sidebar py-4"
            ...
            <div class="sidebar-header flex justify-between items-center">
                <div class="flex items-center space-x-3">
                    ...
                    <div class="user-avatar">
                        ...
                        @else
                            <div class="h-10 w-10 rounded-full bg-gradient-to-br from-teal-400 to-teal-600 flex items-center justify-center text-white font-bold text-lg shadow-lg">
                    ...
                    <div class="user-info sidebar-text">
                        ...
                        @if(Auth::user()->prenom)
                            <p class="text-sm text-teal-100 truncate opacity-90">{{ Auth::user()->prenom }}</p>
                        @endif
                    </div>
                </div>
            </div>
            ...
            <nav class="mt-8 px-2">
                <ul class="space-y-2">
                    <li class="nav-item {{ request()->routeIs('agent.dashboard') ? 'active' : '' }}" data-tooltip="Dashboard">
                        <a href="{{ route('agent.dashboard') }}" 
                           class="{{ request()->routeIs('agent.dashboard') ? 'active-menu' : '' }}">
                    ...
                    <li class="nav-item {{ request()->routeIs('agent.coéquipiers') ? 'active' : '' }}" data-tooltip="Coéquipiers">
                        <a href="{{ route('agent.coéquipiers') }}" 
                           class="{{ request()->routeIs('agent.coéquipiers') ? 'active-menu' : '' }}">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                <path d="M13 6a3 3 0 11-6 0 3 3 0 016 0zM18 8a2 2 0 11-4 0 2 2 0 014 0zM14 15a4 4 0 00-8 0v3h8v-3zM6 8a2 2 0 11-4 0 2 2 0 014 0zM16 18v-3a5.972 5.972 0 00-.75-2.906A3.005 3.005 0 0119 15v3h-3zM4.75 12.094A5.973 5.973 0 004 15v3H1v-3a3 3 0 013.75-2.906z" />
                            </svg>
                            <span class="sidebar-text font-medium">Coéquipiers</span>
                        </a>
                    </li>
                    <li class="nav-item {{ request()->routeIs('agent.reclamations') ? 'active' : '' }}" data-tooltip="Réclamations">
                        <a href="{{ route('agent.reclamations') }}" 
                           class="{{ request()->routeIs('agent.reclamations') ? 'active-menu' : '' }}">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                ...
                <div class="flex justify-between items-center px-6">
                    <div class="flex items-center">
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            <div>
                                <h1 class="text-xl font-bold text-gray-900">@yield('title', 'Dashboard Agent')</h1>
                                <p class="text-sm text-gray-500">Gestion des réclamations et suivi</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <div class="flex items-center space-x-2">
                            <button class="p-2 text-gray-500 hover:text-teal-600 hover:bg-teal-50 rounded-xl transition-all duration-200">
                        ...
                        <div class="relative" x-data="notifications">
                            <button @click="notificationsOpen = !notificationsOpen" 
                                    class="relative p-2 text-gray-500 hover:text-teal-600 hover:bg-teal-50 rounded-xl transition-all duration-200">
                                ...
                                <div class="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-teal-500 to-teal-600">
                                    <div class="flex items-center justify-between">
                                        ...
                                        <button @click="markAllAsRead" 
                                                class="text-teal-100 hover:text-white text-sm font-medium px-3 py-1 rounded-lg hover:bg-white/20 transition-colors">
                                            Tout marquer
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div class="max-h-80 overflow-y-auto custom-scrollbar">
                                    ...
                                    <template x-for="notification in notifications" :key="notification.id">
                                        <div @click="navigateToReclamation(notification.reclamation_id, notification.id, notification.db_id)"
                                             :class="{'bg-teal-50 border-l-4 border-teal-500': !notification.etat}"
                                             class="p-4 hover:bg-gray-50 cursor-pointer border-b border-gray-50 transition-colors">
                                            <div class="flex items-start space-x-3">
                                                <div class="flex-shrink-0">
                                                    <div class="w-10 h-10 rounded-full bg-gradient-to-br from-teal-400 to-teal-600 flex items-center justify-center text-white font-bold text-sm">
                                                        <span x-text="notification.user_name.charAt(0).toUpperCase()"></span>
                                                    </div>
                                                </div>
                                                <div class="flex-1 min-w-0">
                                                    <p class="text-sm font-semibold text-gray-900 mb-1" x-text="notification.message || (notification.user_name + ' a commenté une réclamation')"></p>
                                                    ...
                                                    <div class="flex items-center justify-between">
                                                        ...
                                                        <span x-show="!notification.etat" class="w-2 h-2 bg-teal-500 rounded-full"></span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <button @click="open = !open" 
                                ...
                                <div class="flex items-center space-x-3">
                                    @if(Auth::user()->image)
                                        <img src="{{ asset('images/' . Auth::user()->image) }}" 
                                             alt="{{ Auth::user()->nom }}"
                                             class="h-9 w-9 rounded-xl object-cover ring-2 ring-teal-500">
                                    @else
                                        <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-teal-500 to-teal-600 flex items-center justify-center text-white font-bold">
                                            {{ strtoupper(substr(Auth::user()->nom, 0, 1)) }}
                                        </div>
                                    @endif
                            ...
                            <div x-show="open" 
                                ...
                                <div class="px-4 py-3 border-b border-gray-100">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        @else
                                            <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-teal-500 to-teal-600 flex items-center justify-center text-white font-bold text-lg">
                                                {{ strtoupper(substr(Auth::user()->nom, 0, 1)) }}
                                            </div>
                                        @endif
                                        <div>
                                            <p class="font-semibold text-gray-900">{{ Auth::user()->nom }} {{ Auth::user()->prenom }}</p>
                                            <p class="text-sm text-gray-500">Agent de support</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="py-2">
                                    ...
                                    <a href="#" class="flex items-center px-4 py-3 text-sm text-gray-700 hover:bg-teal-50 hover:text-teal-700 transition-colors">
                                        <svg class="w-5 h-5 mr-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
